...
namespace FileSystem.Fat
{
	...
	public unsafe struct BootSector
	{
		...
		/// </summary>
		[FieldOffset(0x47)]
		private fixed byte _volumeLabel[11];
		...
		/// </summary>
		[FieldOffset(0x52)]
		private fixed byte _fileSystemType[8];
		...
		public uint GetFirstDataSectorNumber()
		{
			try
			{
				uint fatSize = BiosParameterBlock.FatSize16 == 0
					? FatSize32
					: BiosParameterBlock.FatSize16;

				return BiosParameterBlock.ReservedSectorsCount + BiosParameterBlock.FatCount * fatSize;
			}
			catch
			{
				return 0;
		...
		{
			return ((clusterNumber - 2) * BiosParameterBlock.SectorsPerCluster) + GetFirstDataSectorNumber();
		}
		...
		{
			return GetFirstSectorNumber(RootDirFirstClusterNumber) * BiosParameterBlock.BytesPerSector;
		}
		...
		{
			uint dataSectorsCount = BiosParameterBlock.TotalSectors32 - (BiosParameterBlock.ReservedSectorsCount + (BiosParameterBlock.FatCount * FatSize32));

			uint clustersCount = dataSectorsCount / BiosParameterBlock.SectorsPerCluster;
